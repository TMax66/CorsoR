...
dt %>% mutate(st = paste0("ST",st)) %>%
  ...
  theme_bw()+
  labs(y = "N. of isolates", x = "sequence types (ST)") # usate facet_grid per rifare il grafico
...
dt %>% mutate(st = paste0("ST",st)) %>%
  ...
  coord_flip()+
  theme_bw()+
  labs(y = "N. of isolates", x = "Source") # aggiungete un etichetta con il n. di isolati per source
...
crosstable(dt,cols = c(37, 39, 41, 44, 48,50, 51, 52, 53, 54),
           by = Season,test = TRUE,
           total = "column",
           showNA = "ifany") %>%
  as_flextable() #%>%
...
crosstable(dt,cols = c(38, 40, 42, 43, 45, 49 ),
           by = Season,test = TRUE,
           total = "column",
           showNA = "ifany") %>%
  as_flextable() #%>%
